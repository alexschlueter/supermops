...
class SeparableConeSolver:
    def __init__(self, disc, Msys, y, l1_target, nonneg=True):
        ...
        Arguments:
            disc {Discretization} -- discretization for dim reduced problem
    ...
    def solve(self, backend="mosek"):
        ...
        if backend == "cvxopt":
            ...
            sol = np.array(opt_res["x"][-self.disc.total_dofs:]).flatten(
            ...
            sol = np.array(primal[-self.disc.total_dofs:]).flatten(
            )  # first entries are auxiliary vars
        else:
